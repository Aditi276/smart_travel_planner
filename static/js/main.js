...
async function findRoute() {
    ...
    fetchAndDisplayWeather(source, destination);
...
async function fetchAndDisplayWeather(source, destination) {
    try {
        const response = await fetch('/route', {
            method: 'POST',
            headers: { 'Content-Type': 'application/json' },
        ...
        if (data.weather && data.weather.source) {
            const sourceWeather = data.weather.source;
            weatherHtml += `
        ...
        weatherHtml += `<div class="weather-box">
            ...
            <div class="weather-details">`;
        if (data.weather && data.weather.destination) {
            const destWeather = data.weather.destination;
            weatherHtml += `
        ...
        document.getElementById('weather-info').innerHTML = '<p class="error">Could not fetch weather information.</p>';
    }
}
